...
const char *idToEventType[] = {
    "EVENT_TYPE_EMOTE", "EVENT_TYPE_ITEM_USE", "EVENT_TYPE_ITEM_EXPIRE",
...
static bool removeQuestFromDB() {
    ...
    PreparedStatement<WorldDatabaseConnection> *stmt = WorldDatabase.GetPreparedStatement(
        WORLD_DEL_QUEST_CREATURE_STARTER);
...
static bool addQuestToDB() {
    ...
    stmt->setInt8(3, 1);
    stmt->setInt32(14, 9);
    stmt->setString(74, "Title");
    stmt->setString(75, "Objectives.");
    stmt->setString(76, "Details.");
    stmt->setString(78, "Completed.");
    stmt->setInt32(87, 6260);
    stmt->setInt8(93, 2);
    trans->Append(stmt);
...
static bool updateCreature(uint64 guid) {
    ...
    for (std::unordered_map<uint32, WorldSession *>::const_iterator itr = sessions.begin(); itr != sessions.end(); ++
         itr) {
...
static bool createGameObject(int objectId, int mapId, float x, float y, float z, float o) {
    ...
    if (objectInfo->displayId && !sGameObjectDisplayInfoStore.LookupEntry(objectInfo->displayId)) {
        ...
        TC_LOG_ERROR("sql.sql", "Gameobject (Entry %u GoType: %u) have invalid displayId (%u), not spawned.", objectId,
                     objectInfo->type, objectInfo->displayId);
    ...
    TC_LOG_INFO("server.loading", "LANG_GAMEOBJECT_ADD %d %s %d %f %f %f", objectId, objectInfo->name.c_str(), guidLow,
                x, y, z);
    return true;
}
...
void processActions(rapidjson::Document &d) {
    for (rapidjson::Value::ConstValueIterator itr = d["actions"].Begin(); itr != d["actions"].End(); ++itr) {
        ...
        for (std::unordered_map<uint32, WorldSession *>::const_iterator itr = sessions.begin();
            ...
            if (Player *p = ws->GetPlayer()) {
                ...
                if (!strcmp(p->GetName().c_str(), action["player"]["name"].GetString())) {
                    if (!strcmp(actionId, "turn")) {
                        ...
                        o = (o + action["delta_orientation"].GetFloat());
                        if (o < 0) {
                            o = o + G3D::twoPi();
                        }
                        if (o > G3D::twoPi()) {
                        ...
                        p->SetOrientation(o);
                        p->SetFacingTo(o);
                    } else if (!strcmp(actionId, "forward")) {
                        ...
                        const ObjectGuid guid(static_cast<HighGuid>(action["creature"]["hi"].GetUint()),
                                              ...
                                              action["creature"]["counter"].GetUint());
                        p->SetSelection(guid);
                        p->Attack(ObjectAccessor::GetCreatureOrPetOrVehicle(*p, guid), true);
                    }
                }
            }
        }
...
void *processMessages(void *) {
    ...
    if (size > 0 && checkPortTCP(3000, "conciens.mooo.com")) {
        RestClient::Response r = RestClient::post("http://conciens.mooo.com:3000/event", "text/json",
                                                  buffer.GetString());
        try {
...
void EventBridge::sendEvent(const int event_type, const Player *player, const Creature *creature,
                            const uint32 num, const Item *item, const Quest *quest, const SpellCastTargets *targets,
    ...
    if (player != NULL) {
        ...
        jsonPlayer.AddMember("hi", static_cast<uint32>(player->GetGUID().GetHigh()), a);
        jsonPlayer.AddMember("entry", player->GetGUID().GetEntry(), a);
        jsonPlayer.AddMember("counter", player->GetGUID().GetCounter(), a);
    ...
    if (actor != NULL) {
        ...
        jsonActor.AddMember("hi", static_cast<uint32>(actor->GetGUID().GetHigh()), a);
        jsonActor.AddMember("entry", actor->GetGUID().GetEntry(), a);
    ...
    if (creature != NULL) {
        ...
        jsonCreature.AddMember("hi", static_cast<uint32>(creature->GetGUID().GetHigh()), a);
        jsonCreature.AddMember("entry", creature->GetGUID().GetEntry(), a);
        jsonCreature.AddMember("counter", creature->GetGUID().GetCounter(), a);
    ...
    if (targets != NULL && targets->GetObjectTarget() != NULL) {
        ...
        jsonTarget.AddMember("hi", static_cast<uint32>(targets->GetObjectTarget()->GetGUID().GetHigh()), a);
        jsonTarget.AddMember("entry", targets->GetObjectTarget()->GetGUID().GetEntry(), a);
        jsonTarget.AddMember("counter", targets->GetObjectTarget()->GetGUID().GetCounter(), a);
